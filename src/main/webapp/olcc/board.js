...
Board.prototype.updateCSS = function () {
    BoardUpdateCSS(this);
};
...
Board.prototype.loadConfig = function () {
    BoardLoadConfig(this);
};
...
Board.prototype.saveConfig = function () {
    BoardSaveConfig(this);
};
...
function BoardProcessBackend(board, xhr) {
    try {
        ...
        board.timer = window.setTimeout(function () {
            BoardRefresh(board);
        }, board.delay);
        return;
    }
...
Board.prototype.process = function (xhr) {
    BoardProcessBackend(this, xhr);
};

//TODO: paramétriser l'url avec le private=true, dans le get et post
//puis différencier avec une filtre de route
function BoardGetBackend(board) {
    ...
    var get_url = '/t/' + board.name + '/remote.xml?r=' + Math.random();
    ...
    if (board.perso) {
        get_url += '&getUrl=' + encodeURIComponent(board.getUrl);
        get_url += '&postUrl=' + encodeURIComponent(board.postUrl);
        get_url += '&postData=' + encodeURIComponent(board.postData);
        get_url += '&slip=' + encodeURIComponent(board.slip);
    }

    get_url += '&last=' + board.lastId
    xhr.open('GET', get_url, true);
    if (board.perso) xhr.setRequestHeader('X-Olccs-Private', '1');
...
Board.prototype.getBackend = function () {
    BoardGetBackend(this);
};
...
function BoardPost(board, msg) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('POST', '/t/'+board.name+'/post', true);
    if (board.perso) xhr.setRequestHeader('X-Olccs-Private', '1');
    xhr.onreadystatechange = function () {
        switch (xhr.readyState) {
            case 4:
...
Board.prototype.postResult = function (xhr) {
    BoardPostResult(this, xhr);
};
...
BoardTab.prototype.addTab = function () {
    BoardTabAddTab(this);
};
...
BoardTab.prototype.notified = function (notif, arg) {
    BoardTabNotified(this, notif, arg);
};
...
BoardTab.prototype.display = function () {
    BoardTabDisplay(this);
};
...
BoardTab.prototype.hide = function () {
    BoardTabHide(this);
};
...
BoardTab.prototype.toggle = function () {
    BoardTabToggle(this);
};
...
BoardTab.prototype.removeTab = function () {
    return BoardTabRemoveTab(this);
};
...
Board.prototype.configPanel = function () {
    return BoardConfigPanel(this);
};
